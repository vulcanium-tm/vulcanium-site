package dev.vulcanium.site.tech.store.controller.order.facade;
...
import dev.vulcanium.business.services.payments.TransactionService;
...
import dev.vulcanium.business.model.payments.CreditCardPayment;
...
import dev.vulcanium.site.tech.populator.customer.PersistableCustomerPopulator;
import dev.vulcanium.site.tech.populator.order.OrderProductPopulator;
import dev.vulcanium.site.tech.populator.order.PersistableOrderApiPopulator;
import dev.vulcanium.site.tech.populator.order.ReadableOrderPopulator;
import dev.vulcanium.site.tech.populator.order.ReadableOrderProductPopulator;
import dev.vulcanium.site.tech.populator.order.ShoppingCartItemPopulator;
...
private Order processOrderModel(ShopOrder order, Customer customer, Transaction transaction, MerchantStore store,
	...
	try {
		...
		if (PaymentType.PAYPAL.name().equals(paymentType)) {
			...
			payment = new dev.vulcanium.business.model.payments.PaypalPayment();
			
			((dev.vulcanium.business.model.payments.PaypalPayment) payment)
...
@Override
public ReadableOrderList getReadableOrderList(OrderCriteria criteria,
                                                                                   MerchantStore store) {
...
private ReadableOrderList getReadableOrderList(OrderCriteria criteria,
                                                                                    MerchantStore store, Language language) throws Exception {
...
@Override
public ReadableOrderList getReadableOrderList(MerchantStore store, int start,
...
@Override
public ReadableOrderList getCapturableOrderList(MerchantStore store,
