package dev.vulcanium.site.tech.store.controller.shoppingcart.facade;
...
import dev.vulcanium.site.tech.mapper.cart.ReadableShoppingCartMapper;
import dev.vulcanium.site.tech.model.customer.Customer;
import dev.vulcanium.site.tech.model.shoppingcart.*;
import dev.vulcanium.site.tech.populator.shoppingCart.ShoppingCartDataPopulator;
...
public ShoppingCartData addItemsToShoppingCart(final ShoppingCartData shoppingCartData, final ShoppingCartItem item,
	...
	dev.vulcanium.business.model.shoppingcart.ShoppingCartItem shoppingCartItem = createCartItem(cartModel, item,
			store);
	
	boolean duplicateFound = false;
	if (CollectionUtils.isEmpty(item.getShoppingCartAttributes())) {
		// get duplicate item from the cart
		Set<dev.vulcanium.business.model.shoppingcart.ShoppingCartItem> cartModelItems = cartModel.getLineItems();
		for (dev.vulcanium.business.model.shoppingcart.ShoppingCartItem cartItem : cartModelItems) {
...
private dev.vulcanium.business.model.shoppingcart.ShoppingCartItem createCartItem(final ShoppingCart cartModel,
	...
	dev.vulcanium.business.model.shoppingcart.ShoppingCartItem item = shoppingCartService
	...
	if (!CollectionUtils.isEmpty(cartAttributes)) {
		for (ShoppingCartAttribute attribute : cartAttributes) {
			...
			if (productAttribute != null
				...
				dev.vulcanium.business.model.shoppingcart.ShoppingCartAttributeItem attributeItem = new dev.vulcanium.business.model.shoppingcart.ShoppingCartAttributeItem(
...
// KEEP -- ENTRY
private dev.vulcanium.business.model.shoppingcart.ShoppingCartItem createCartItem(ShoppingCart cartModel,
	...
	// set attributes
	List<dev.vulcanium.site.tech.model.catalog.product.attribute.ProductAttribute> attributes = shoppingCartItem
			                                                                                          .getAttributes();
	if (!CollectionUtils.isEmpty(attributes)) {
		for (dev.vulcanium.site.tech.model.catalog.product.attribute.ProductAttribute attribute : attributes) {
...
// used for api
private List<dev.vulcanium.business.model.shoppingcart.ShoppingCartItem> createCartItems(ShoppingCart cartModel,
	...
	List<dev.vulcanium.business.model.shoppingcart.ShoppingCartItem> items = new ArrayList<>();
...
private dev.vulcanium.business.model.shoppingcart.ShoppingCartItem getEntryToUpdate(final long entryId,
                                                                                   final ShoppingCart cartModel) {
	if (CollectionUtils.isNotEmpty(cartModel.getLineItems())) {
		for (dev.vulcanium.business.model.shoppingcart.ShoppingCartItem shoppingCartItem : cartModel
				                                                                                  .getLineItems()) {
...
public ShoppingCartData removeCartItem(final Long itemID, final String cartId, final MerchantStore store,
	...
	if (StringUtils.isNotBlank(cartId)) {
		...
		if (cartModel != null) {
			if (CollectionUtils.isNotEmpty(cartModel.getLineItems())) {
				Set<dev.vulcanium.business.model.shoppingcart.ShoppingCartItem> shoppingCartItemSet = new HashSet<dev.vulcanium.business.model.shoppingcart.ShoppingCartItem>();
				for (dev.vulcanium.business.model.shoppingcart.ShoppingCartItem shoppingCartItem : cartModel
						                                                                                  .getLineItems()) {
...
public ShoppingCartData updateCartItem(final Long itemID, final String cartId, final long newQuantity,
	...
	if (StringUtils.isNotBlank(cartId)) {
		ShoppingCart cartModel = getCartModel(cartId, store);
		if (cartModel != null) {
			dev.vulcanium.business.model.shoppingcart.ShoppingCartItem entryToUpdate = getEntryToUpdate(
...
public ShoppingCartData updateCartItems(Optional<String> promoCode, final List<ShoppingCartItem> shoppingCartItems,
	...
	ShoppingCart cartModel = null;
	Set<dev.vulcanium.business.model.shoppingcart.ShoppingCartItem> cartItems = new HashSet<dev.vulcanium.business.model.shoppingcart.ShoppingCartItem>();
	for (ShoppingCartItem item : shoppingCartItems) {
		...
		dev.vulcanium.business.model.shoppingcart.ShoppingCartItem entryToUpdate = getEntryToUpdate(item.getId(),
				cartModel);
...
public @Nullable ReadableShoppingCart removeShoppingCartItem(String cartCode, String sku,
	...
	Set<dev.vulcanium.business.model.shoppingcart.ShoppingCartItem> items = new HashSet<dev.vulcanium.business.model.shoppingcart.ShoppingCartItem>();
	dev.vulcanium.business.model.shoppingcart.ShoppingCartItem itemToDelete = null;
	for (dev.vulcanium.business.model.shoppingcart.ShoppingCartItem shoppingCartItem : cart.getLineItems()) {
...
private ReadableShoppingCart readableShoppingCart(ShoppingCart cartModel, PersistableShoppingCartItem item,
	...
	dev.vulcanium.business.model.shoppingcart.ShoppingCartItem itemModel = createCartItem(cartModel, item, store);
...
private ReadableShoppingCart modifyCart(ShoppingCart cartModel, PersistableShoppingCartItem item,
	...
	Set<dev.vulcanium.business.model.shoppingcart.ShoppingCartItem> items = cartModel.getLineItems();
	if (!CollectionUtils.isEmpty(items)) {
		Set<dev.vulcanium.business.model.shoppingcart.ShoppingCartItem> newItems = new HashSet<dev.vulcanium.business.model.shoppingcart.ShoppingCartItem>();
		Set<dev.vulcanium.business.model.shoppingcart.ShoppingCartItem> removeItems = new HashSet<dev.vulcanium.business.model.shoppingcart.ShoppingCartItem>();
		for (dev.vulcanium.business.model.shoppingcart.ShoppingCartItem anItem : items) {// take care of existing
			// product
		...
		if (!removeItems.isEmpty()) {
			for (dev.vulcanium.business.model.shoppingcart.ShoppingCartItem emptyItem : removeItems) {
...
private ReadableShoppingCart modifyCartMulti(ShoppingCart cartModel, List<PersistableShoppingCartItem> cartItems,
	...
	List<dev.vulcanium.business.model.shoppingcart.ShoppingCartItem> inCartItemList = createCartItems(cartModel,
			cartItems, store);
	
	Set<dev.vulcanium.business.model.shoppingcart.ShoppingCartItem> existingItems = cartModel.getLineItems();
	// loop over incoming items since they drive changes
	for (dev.vulcanium.business.model.shoppingcart.ShoppingCartItem newItemValue : inCartItemList) {
		
		// check that item exist in persisted cart
		Optional<dev.vulcanium.business.model.shoppingcart.ShoppingCartItem> oOldItem = existingItems.stream()
				                                                                               .filter(i -> i.getSku().equals(newItemValue.getSku())
				                                                                               
				                                                                               ).findFirst();
		
		if (oOldItem.isPresent()) {
			dev.vulcanium.business.model.shoppingcart.ShoppingCartItem oldCartItem = oOldItem.get();
